...
const scrapeProject=async (id:string):Promise<ProjectDetails>=>{
    const graphqlQuery=fs.readFileSync("src/behance/behanceProject.graphql","utf8").replaceAll("\n","  ")
    ...
    const projectDetails=await fetch("https://www.behance.net/v3/graphql", {
        ...
        "method": "POST"
    }).then(e => e.text()).then(e => JSON.parse(e)) as { data:ProjectDetails };
    return projectDetails.data
}
const scrapeProjects= async (): Promise<Project[]> => {
    try {
        const graphqlQuery=fs.readFileSync("src/behance/behanceList.graphql","utf8").replaceAll("\n","  ")
        ...
        while (true) {
            const {data} = await fetch("https://www.behance.net/v3/graphql", {
                ...
                "method": "POST"
            }).then(e => e.text()).then(e => JSON.parse(e)) as { data: Project[] };
